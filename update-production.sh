...
echo "1. ÉTAPE CRITIQUE - Appliquer le correctif SQL à la base de données :"
echo "   docker-compose exec logiflow-db psql -U logiflow_admin -d logiflow_db -f /path/to/apply-production-fix.sql"
echo "   OU se connecter directement :"
echo "   docker-compose exec logiflow-db psql -U logiflow_admin -d logiflow_db"
echo "   puis exécuter : ALTER TABLE users ADD COLUMN name VARCHAR(255);"
echo ""
echo "2. Récupérez les fichiers corrigés :"
echo "   - server/storage.production.ts (méthodes en SQL brut)"
echo "   - shared/schema.ts (schéma corrigé)"
echo ""
echo "3. Reconstruisez l'image Docker :"
