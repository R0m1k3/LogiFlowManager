...
var init_storage_production = __esm({
  "server/storage.production.ts"() {
    ...
    DatabaseStorage = class {
      async getUser(id) {
        const { pool: pool2 } = await Promise.resolve().then(() => (init_db_production(), db_production_exports));
        const result = await pool2.query(`SELECT * FROM users WHERE id = $1`, [id]);
      ...
      async getUserByUsername(username) {
        const { pool: pool2 } = await Promise.resolve().then(() => (init_db_production(), db_production_exports));
        const result = await pool2.query(`SELECT * FROM users WHERE username = $1`, [username]);
        return result.rows[0];
      }
      async upsertUser(userData) {
        ...
        WHERE id = $6
        RETURNING *
      `, [userData.username, userData.email, userData.name, userData.role, /* @__PURE__ */ new Date(), existingUser.id]);
          return result.rows[0];
        } else {
          const result = await pool2.query(`
      ...
      async getUserWithGroups(id) {
        try {
          const { pool: pool2 } = await Promise.resolve().then(() => (init_db_production(), db_production_exports));
          const userResult = await pool2.query(`SELECT * FROM users WHERE id = $1`, [id]);
    ...
    `, [userData.id, userData.username, userData.email, userData.name, userData.role, userData.password, userData.passwordChanged, /* @__PURE__ */ new Date(), /* @__PURE__ */ new Date()]);
      ...
      WHERE id = $8
      RETURNING *
    `, [userData.username, userData.email, userData.name, userData.role, userData.password, userData.passwordChanged, /* @__PURE__ */ new Date(), id]);
        return result.rows[0];
      }
      async deleteUser(id) {
        const { pool: pool2 } = await Promise.resolve().then(() => (init_db_production(), db_production_exports));
      ...
      async getGroups() {
        const { pool: pool2 } = await Promise.resolve().then(() => (init_db_production(), db_production_exports));
        const result = await pool2.query(`SELECT * FROM groups ORDER BY name`);
        return result.rows;
      }
    ...
    `, [group.name, group.color, /* @__PURE__ */ new Date(), id]);
      ...
      async getSuppliers() {
        const { pool: pool2 } = await Promise.resolve().then(() => (init_db_production(), db_production_exports));
        const result = await pool2.query(`SELECT * FROM suppliers ORDER BY name`);
        return result.rows;
      }
      ...
      VALUES ($1, $2, $3, $4, $5)
      RETURNING *
    `, [supplier.name, supplier.contact, supplier.phone, /* @__PURE__ */ new Date(), /* @__PURE__ */ new Date()]);
        return result.rows[0];
      }
      ...
      WHERE id = $5
      RETURNING *
    `, [supplier.name, supplier.contact, supplier.phone, /* @__PURE__ */ new Date(), id]);
        return result.rows[0];
      }
    ...
    `, [id]);
      ...
      RETURNING *
    `, [order.supplierId, order.groupId, order.plannedDate, order.status, order.notes, order.createdBy, /* @__PURE__ */ new Date(), /* @__PURE__ */ new Date()]);
        return result.rows[0];
      }
      ...
      WHERE id = $7
      RETURNING *
    `, [order.supplierId, order.groupId, order.plannedDate, order.status, order.notes, /* @__PURE__ */ new Date(), id]);
        return result.rows[0];
      }
    ...
    `, [id]);
      ...
      RETURNING *
    `, [delivery.orderId, delivery.supplierId, delivery.groupId, delivery.scheduledDate, delivery.quantity, delivery.unit, delivery.status, delivery.notes, delivery.createdBy, /* @__PURE__ */ new Date(), /* @__PURE__ */ new Date()]);
        return result.rows[0];
      }
      ...
      WHERE id = $10
      RETURNING *
    `, [delivery.orderId, delivery.supplierId, delivery.groupId, delivery.scheduledDate, delivery.quantity, delivery.unit, delivery.status, delivery.notes, /* @__PURE__ */ new Date(), id]);
        return result.rows[0];
      }
      ...
      async getUserGroups(userId) {
        const { pool: pool2 } = await Promise.resolve().then(() => (init_db_production(), db_production_exports));
        const result = await pool2.query(`SELECT * FROM user_groups WHERE user_id = $1`, [userId]);
        return result.rows;
      }
    ...
    `, [userGroup.userId, userGroup.groupId, /* @__PURE__ */ new Date()]);
      ...
      async removeUserFromGroup(userId, groupId) {
        const { pool: pool2 } = await Promise.resolve().then(() => (init_db_production(), db_production_exports));
        await pool2.query(`DELETE FROM user_groups WHERE user_id = $1 AND group_id = $2`, [userId, groupId]);
      }
    ...
    `, params);
        const deliveriesResult = await pool2.query(`
      SELECT COUNT(*) as count FROM deliveries 
      WHERE scheduled_date >= $1 AND scheduled_date <= $2${whereClause}
    `, params);
        const pendingOrdersResult = await pool2.query(`
      SELECT COUNT(*) as count FROM orders 
