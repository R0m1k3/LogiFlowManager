...
export class DatabaseStorage implements IStorage {
  ...
  async createOrder(order: InsertOrder): Promise<Order> {
    ...
    const result = await pool.query(`
      ...
      RETURNING *
    `, [order.supplierId, order.groupId, order.plannedDate, order.status, order.notes, order.createdBy, new Date(), new Date()]);
    return result.rows[0];
  }
  ...
  async createDelivery(delivery: InsertDelivery): Promise<Delivery> {
    ...
    const result = await pool.query(`
      ...
      RETURNING *
    `, [delivery.orderId, delivery.supplierId, delivery.groupId, delivery.scheduledDate, delivery.quantity, delivery.unit, delivery.status, delivery.notes, delivery.createdBy, new Date(), new Date()]);
    return result.rows[0];
  }
